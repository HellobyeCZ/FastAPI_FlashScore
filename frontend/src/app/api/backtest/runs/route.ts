...
export async function GET(request: Request) {
  ...
  const base = `${resolveBackendBaseUrl()}/backtest/runs`;
...
export async function POST(request: Request) {
  const backendUrl = `${resolveBackendBaseUrl()}/backtest/runs`;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to create backtest run.";
    return NextResponse.json(
      {
        error: {
          code: "backtest_run_create_failed",
          message
        }
      },
      { status: 502 }
